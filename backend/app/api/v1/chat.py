...
2. Only record information the student EXPLICITLY stated or directly confirmed.
   - "I always mix up X and Y" → valid weak_point
   - "I prefer visual explanations" → valid learning_preference
   - "I'm preparing for the final exam" → valid learning_goal
   - A one-off knowledge question like "what is X?" → do NOT infer the student doesn't know X
3. memory_type MUST be one of:
   learning_goal, knowledge_level, weak_point, learning_preference, course_context, personal_fact
4. importance must be between 0.3 and 1.0. Higher = more valuable to remember long-term.
5. content must be a SHORT, ATOMIC, SELF-CONTAINED statement about the student (≤150 characters).
   Use consistent phrasing. Prefer simple declarative sentences.
   Example good content: "容易混淆平稳增量与独立增量"
   Example bad content: "学生总是分不清平稳增量和独立增量这两个概念，希望在后续讲解中多做对比。"

DO NOT RECORD under any circumstances:
- Casual chat, thanks, greetings, one-off factual questions
...
- Speculation or guesses about the student's knowledge level from a single question
- What the AI tutor said — only record what the STUDENT revealed about themselves
